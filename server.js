...
// API: AI чат с рекомендациями — ФИКС: HISTORY + ВАРИАЦИИ CTA
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (history.length > 5) history = history.slice(-5); // Храним последние 5 сообщений
    ...
    const productText = productCount > 0 ? 
      `\n\n**📦 РЕКОМЕНДАЦИИ ИЗ КАТАЛОГА ЭНТЕХ (${productCount} шт.):**\n` +
      products.map((p, i) => 
...
**ПРАВИЛА:**
1. **ВСЕГДА рекомендуй 2 товара** из каталога при упоминании типа помещения, IP, мощности или площади.
2. Если клиент упоминает: склад/цех → промышленные IP65 150-300Вт; офис → офисные IP20 30-60Вт; улица → уличные IP65+ 50-150Вт.
3. **Задавай вопросы ТОЛЬКО если не хватает деталей** — максимум 1 уточнение, потом рекомендации.
4. **Цены НЕ называй** — "менеджер рассчитает индивидуально".
5. **Всегда заканчивай CTA**: ВАРЬИРУЙ: "Хотите КП в PDF?" или "Хотите расчет освещенности?" (чередуй случайно).
6. **Если просят фото**: "Фото светильников смотрите на сайте Энтех или запросите у менеджера в КП".

**ФОРМАТ ОТВЕТА:**
- **Введение**: "Для [помещение] рекомендую..."
- **Рекомендации**: 2 модели с краткими характеристиками (модель, Вт, лм, IP, гарантия).
...
app.listen(PORT, () => logger.info(`Server on :${PORT}`));import express from "express";
import dotenv from "dotenv";
import fs from "fs/promises";
import cors from "cors";
import helmet from "helmet";
